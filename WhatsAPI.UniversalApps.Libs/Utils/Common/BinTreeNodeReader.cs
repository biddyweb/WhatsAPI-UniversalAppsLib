...
namespace WhatsAPI.UniversalApps.Libs.Utils.Common
{
    public class BinTreeNodeReader
    {
        ...
        protected byte[] readBytes(int token)
        {
            ...
            {
                ret = System.Text.Encoding.GetEncoding(Constants.Information.ASCIIEncoding).GetBytes(this.getToken(token));
            }
            else if (token == 0)
            {
                ret = new byte[0];
            ...
            else if (token == 254)
            {
                ...
                ret = System.Text.Encoding.GetEncoding(Constants.Information.ASCIIEncoding).GetBytes(this.getToken(tmpToken + 0xf5));
            }
            else if (token == 250)
            {
                ...
                {
                    ret = System.Text.Encoding.GetEncoding(Constants.Information.ASCIIEncoding).GetBytes(server);
                }
            }
            return ret;
        }
        ...
        protected IEnumerable<KeyValue> readAttributes(int size)
        {
            ...
            for (int i = 0; i < attribCount; i++)
            {
                ...
                string key = System.Text.Encoding.GetEncoding(Constants.Information.ASCIIEncoding).GetString(keyB, 0, keyB.Length);
                string value = System.Text.Encoding.GetEncoding(Constants.Information.ASCIIEncoding).GetString(valueB,0,valueB.Length);
